...
## Vue Directive
- v- 접두사가 붙은 특수한 HTML 속성으로 Vue 인스턴스와 연동된다.
- Directive 뒤에 오는 값인 v-(명령어)="{표현식}"

| Directive   | 설명                                                | 비고                          |
| ----------- | ------------------------------------------------- | --------------------------- |
| v-html      | 요소의 HTML 콘텐츠를 표현식의 값으로 업데이트                       | xss 공격 유의                   |
...
| v-show      | 표현식의 참/거짓에 따라 HTML element를 보이거나 숨김               | v-if와 구분, display: none; 처리 |
